...
func FindInstallationsWindowsHeroic() ([]*Installation, []error) {
	...
	if err != nil {
		return nil, []error{errors.Wrap(err, "failed to get user config dir")}
	}
	heroicConfigPath := filepath.Join(configDir, "heroic", "config.json")
	heroicGamesConfigPath := filepath.Join(configDir, "heroic", "GamesConfig")

	// Legendary doesn't use UserConfigDir, but UserHomeDir/.config, regardless of OS
