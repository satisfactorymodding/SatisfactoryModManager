...
func FindInstallationsSteam(steamPath string, launcher string, platform common.LauncherPlatform) ([]*common.Installation, []error) {
	...
	for _, steamappsCasing := range steamappsCasings {
		steamAppsPath := filepath.Join(steamPath, steamappsCasing)
		libraryFoldersManifestPath := platform.ProcessPath(filepath.Join(steamAppsPath, "libraryfolders.vdf"))

		thisLibraryFolders, err := getLibraryFoldersFromManifest(libraryFoldersManifestPath)
		if err != nil {
			return nil, []error{fmt.Errorf("failed to get library folders from manifest: %w", err)}
		}
		rawLibraryFolders = append(rawLibraryFolders, thisLibraryFolders...)
	}
	...
	for _, libraryFolder := range libraryFolders {
		for _, steamappsCasing := range steamappsCasings {
			steamappsDir := filepath.Join(libraryFolder, steamappsCasing)
			for _, manifest := range manifests {
				manifestPath := platform.ProcessPath(filepath.Join(steamappsDir, manifest))
				...
				fullInstallationPath := platform.ProcessPath(filepath.Join(steamappsDir, "common", appState["installdir"].(string)))
				...
				if platform.Os() != "windows" {
					...
					gameProtonPrefix := platform.ProcessPath(filepath.Join(steamappsDir, "compatdata", appState["appid"].(string), "pfx"))
